...
public class StreamUtil {
    public static Collection<? extends Streamable> readStreamablesFrom(DataInputStream dis) throws IOException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        ...
        for (int i = 0; i < streamablesSize; ++i) {
            streamables.add(readStreamableFrom( dis));
        }

        return streamables;
    }

    public static Streamable readStreamableFrom(DataInputStream dis) throws IOException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        Streamable streamable = (Streamable) Class.forName(dis.readUTF()).newInstance();
        streamable.readFrom(dis);
        return streamable;
    }

    public static void writeStreamablesTo(DataOutputStream dos, Collection<? extends Streamable> streamables) throws IOException {
        dos.writeUTF(streamables.getClass().getName());
        dos.writeInt(streamables.size());

        for (Streamable streamable : streamables) {
            writeStreamableTo(dos, streamable);
        }
    }

    public static void writeStreamableTo(DataOutputStream dos, Streamable streamable) throws IOException {
        dos.writeUTF(streamable.getClass().getName());
        streamable.writeTo(dos);
    }
